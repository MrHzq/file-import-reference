...
// 全字匹配正则
const fullWordMatchRegex = (word) => new RegExp(`\\.?${word}(?!\\w)`);
...
// 是否有引用关系
const hasReference = (c, word) => matchReg(c, `/${word}`);
        ...
        if (matchReg(content, targetName)) {
          ...
          lines.forEach((lineContent, index) => {
            if (
              ...
              hasReference(lineContent, targetName)
            ) {
              ++matchResultCount;
...
module.exports = async function (options) {
  ...
  if (!fileName) return log.error(`查找内容不能为空!!`);
    ...
    result.forEach((item) => {
      ...
      item.match.forEach((m) => {
        ...
        log.info(
          ...
          )}${m.matchContent.replace(
            getFullWordMatchingRegex(m.targetName),
